...
def validate(xmlString, schema = "http://www.ivoa.net/xml/VOEvent/VOEvent-v2.0.xsd"):
...
def parse(fileordoc):
    ...
    '''
    if isinstance(fileordoc, str):   # RDW Mar 2013
        doc = VOEvent.parsexml_(fileordoc)
    else:
        doc = fileordoc
...
def getWhereWhen(v):
    ...
    instants = time.get_TimeInstant()
    if len(instants) == 1:
        wwd['time'] = instants[0].get_ISOTime()
